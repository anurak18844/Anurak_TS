...
function calBonus(years: number):number{
    if(years<=1) return 0
    else if(years<=3) return 1
    else if(years<=5) return 2
    else return 3
}

function calGift(years: number):string{
    if(years<=3) return "Rice Cooker"
    else return "Microwave Oven"
}

function formatToCurrency(salary:number){
